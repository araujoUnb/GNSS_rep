...
def array_lin_noise(theta_deg, n_antennas,epsilon):
    return array_lin(theta_deg, n_antennas) + sqrt(epsilon/2)*th.exp(1j*th.rand(array_lin(theta_deg, n_antennas).size())*2*pi)
...
class gnssSimulSignal:
    ...
    def signalModel(self,DoA_deg=10,angle_diff_deg=5,delay_diff=0.5):
        ...
        Z = tl.tensor(1/sqrt(2)*th.randn(self.m,self.k,self.n) + 1j*th.randn(self.m,self.k,self.n),dtype=th.complex64)
...
def black_box_LS(Y2, dataModel, tauLos,tauNLos):
    CQw = genCQw(dataModel.B, dataModel.Tc, dataModel.T, dataModel.CB_delays, th.tensor([tauLos,tauNLos]), dataModel.CA_FFT)
    M = np.linalg.pinv(CQw) @ Y2.numpy()

    return  - (np.linalg.norm(Y2.numpy() - CQw@M)/np.linalg.norm(Y2.numpy()))**2

def ref_black_box_LSBayOpt(Y2, dataModel, tau_vec):
    return -black_box_LS(Y2, dataModel, tau_vec[0],tau_vec[1])
...
def esprit(E,L,N):
    ...
    Phi = th.linalg.pinv(S[0: N - 1,:]) @ S[1:N,:] # the original array is divided into two subarrays [0,1,...,N-2] and [1,2,...,N-1]
...
def simulation(seed, angle_diff_deg, delay_diff, C_N0, xi, epsilon):
    ...
    pbounds = {'tauLos':(-dataModel.Tc,dataModel.Tc), 
               'tauNLos':(-dataModel.Tc,dataModel.Tc) }
    ...
    return tau_real.numpy().item(), tau_HOSVD_ideal.numpy().item(), tau_bayOpt_est, tau_bayOpt_ref_est, tau_lskrf_est.numpy().item(), tau_ag_est.numpy().item()
...
if __name__ =="__main__":
    ...
    with shelve.open(f'results/results_{angle_diff_deg}_{delay_diff}_{C_N0}_{xi}_{epsilon}.dat', 'n') as f:
