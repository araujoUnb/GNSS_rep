...
def black_box_LS(Y2, dataModel, tauLos, tauNLos):
    ...
    return -(np.linalg.norm(Y2 - CQw @ M) / np.linalg.norm(Y2.numpy()))**2
...
def hosvd_delay_estimation(Y, dataModel, OMEGA):
    weigthTucker, FactorsTucker = tucker(Y, [2,2,2], n_iter_max=1000, init='svd', svd='randomized_svd', tol=1e-5)
    q_hat = tl.tenalg.mode_dot(tl.tenalg.mode_dot(Y,FactorsTucker[0].T.conj(),0),FactorsTucker[1].T.conj(),1)[0,0,:] @ OMEGA  
    yfit = interpolate_bank_filter(dataModel.CB_delays,abs(q_hat),dataModel.isv)
    return dataModel.isv[th.argmax(yfit)], yfit
...
#################################################### Simulation ######################################################
def bayesian_opt(seed, Y, dataModel, xi):
    ...
    optimizer.maximize(init_points=randomPoints, n_iter=60, acquisition_function=utility)

    tau_vec = np.array([optimizer.max['params']['tauLos'], optimizer.max['params']['tauNLos']])
    tau_bayOpt_est = np.min(tau_vec)
    print(f"Bayesian: {time.perf_counter() - start:.5f}")
    ...
    print(f"Reference: {time.perf_counter() - start:.5f}")

    return tau_bayOpt_est, tau_bayOpt_ref_est
...
def simulation(seed, angle_diff_deg, delay_diff, C_N0, xi, epsilon):
    ...
    #Y_FBA    = tl.fold(Z,0,[dataModel.m,2*dataModel.k,dataModel.q])
    #Y_SPS    = tl.fold(W,0,[m_s,l_s*dataModel.k,dataModel.q])
    Y_FBASPS = tl.fold(E,0,[m_s,2*l_s*dataModel.k,dataModel.q])
    #Y_SPSEXP = tl.fold(W,0,[m_s,dataModel.k,dataModel.q,l_s])  
    #Y_FBASPSEXP = tl.fold(E,0,[m_s,2*dataModel.k,dataModel.q,l_s])  

    tau_real = dataModel.tau_vec[0]
    tau_HOSVD_ideal = ideal_delay_estimation(CQ, dataModel)
    tau_HOSVD, _ = hosvd_delay_estimation(Y,dataModel,OMEGA)
    tau_SPSFBA, _ = hosvd_delay_estimation(Y_FBASPS,dataModel,OMEGA)
    tau_ag_est, _ = known_spatial_phase_factors(Y, factors[0], factors[1], dataModel, OMEGA)

    start = time.perf_counter()
    tau_lskrf_est, _ = esprit_lskrf(Y, E, 2, m_s, dataModel, OMEGA)
    print(f"LSKRF: {time.perf_counter() - start:.5f}")


    tau_bayOpt_est, tau_bayOpt_ref_est = bayesian_opt(seed, Y, dataModel, xi)

    return tau_real.numpy().item(), tau_HOSVD_ideal.numpy().item(), \
        tau_HOSVD.numpy().item(), tau_SPSFBA.numpy().item(), \
        tau_ag_est.numpy().item(), tau_lskrf_est.numpy().item(), \
        tau_bayOpt_est, tau_bayOpt_ref_est
...
if __name__ == "__main__":
    ...
    tau_HOSVD = np.full(mc, np.nan)
    tau_SPSFBA = np.full(mc, np.nan)
    ...
    for seed in range(mc):
        ...
        tau_HOSVD[seed] = results[2]
        tau_SPSFBA[seed] = results[3]
        tau_ag_est[seed] = results[4]
        tau_lskrf_est[seed] = results[5]
        tau_bayOpt_est[seed] = results[6]
        tau_bayOpt_ref_est[seed] = results[7]
    ...
    with shelve.open(f'results/results_{angle_diff_deg}_{delay_diff}_{C_N0}_{xi}_{epsilon}_{batch}.dat', 'n') as f:
        ...
        f['tau_HOSVD'] = tau_HOSVD
        f['tau_SPSFBA'] = tau_SPSFBA
