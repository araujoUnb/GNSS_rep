...
def simulation(seed, angle_diff_deg, delay_diff, C_N0, xi, epsilon):
    ...
    tau_HOSVD       = hosvd_delay_estimation(Y,dataModel,OMEGA)
    tau_SPSFBA      = hosvd_delay_estimation(Y_FBASPS,dataModel,OMEGA)
    ...
    return tau_real.numpy().item(), tau_HOSVD.numpy().item(), \
        tau_SPSFBA.numpy().item()
